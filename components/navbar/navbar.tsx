  ...
  return (
    <>
      <header className="fixed top-0 z-50 w-full bg-white backdrop-blur shadow-sm">
        <nav className="mx-auto max-w-7xl flex h-[5rem] items-center gap-4 px-4 text-slate-700">
          {/* LEFT: LOGO + SEARCH */}
          ...
          <ul className="hidden lg:flex items-center gap-6">
            {menu.map((item, i) => (
              <li key={i}>
                <Link
                  href={item.path}
                  className={`hover:text-e-primary transition ${path === item.path && item.path !== "/" ? "text-e-primary" : ""
                    }`}
                >
                  {item.name}
                </Link>
